...
The bot auto-fills unit price + supplier from your master list and computes totals. **The requester never sees prices or totals** (one exception: when an item exists under several suppliers, the variant picker shows each unit price so they can compare). **The Stock controller is also price-blind** — their card and PDF show items, codes, packs and quantities only (no unit prices, totals, or price-change flags), since their role is verifying the stock need, not the cost. **The Approved POs group post is price-free too** (items and quantities only). Pricing appears to the Bookkeeping/Finance/GM/Board groups, on their PDFs, and on the Cash Advance post. One supplier per PO. A reject at any stage goes back to the requester with the reason; after they fix it, the PO **re-runs from the Stock controller**.
...
- **Master list** — your existing tab (default name `Table1`; change with `MASTER_TAB`). Columns are matched by name, so `No. / Material Code / CML Reagent / Supplier / Supplier Reagent / Price / Pack` works as-is. The requester searches by **CML Reagent**; **Price** is used for totals but never shown to the requester. The bot only reads this tab — it never edits it. **The same CML Reagent may be listed under several suppliers (or packs)**: the bot then shows a picker (supplier · pack · **unit price**, so the requester can compare variants — the only place a requester sees lab prices). If the PO is already locked to a supplier, the matching variant is selected automatically; if none of the variants matches the locked supplier, the bot lists which suppliers carry the item. **If the requester picks a supplier whose price is above another supplier's for the same item, the bot requires a short justification** (out of stock, quality, delivery time, …); the note is stored on the line item (`Line_Items.variant_reason`) and shown to the price-visible approvers on the card and PDF page 2. Choosing a bigger pack from the *same* supplier does not trigger the prompt.
...
- **Other_Items** — created automatically; a reusable list for the **Other** category (columns: item, unit_price, supplier, updated_at, updated_by). The bot suggests from it and appends new entries you add on the fly.

### "Other" price control & supplier dedup

- **Reusing an Other item**: the bot shows its **last price** with buttons **✅ Use $x** / **✏️ Enter new price**. A new price within `OTHER_PRICE_TOLERANCE_PCT` (default ±20%) is accepted with a small "(was $x)" note on the card; **at/above the tolerance the requester must confirm**, and approvers see a **⚠️ price changed** flag on the chat card and on PDF page 2 (never on the supplier copy).
- The reference price in `Other_Items` is updated **only when the PO is submitted** (with `updated_at` / `updated_by`). The price actually used on each PO stays in `Line_Items` (`unit_price` vs `ref_price`), so the full history is auditable per PO.
- **New Other item → supplier**: the bot offers a **pick-list of known suppliers** first. A typed name that matches an existing one apart from case/punctuation ("abc co." vs "ABC Co") is **merged automatically**; a close-but-not-equal name triggers a "same supplier?" confirmation — so the list doesn't fill up with near-duplicates. If the PO is already locked to a supplier, the question is skipped.
